...
def write_to_redis(record):
    ...
    try:
        ...
        r.set(f"top_products:{product_id}", count)
    except:
        pass
...
def main():
    ...
    purchase_stream = stream \
        ...
        .filter(lambda data: data.get("event_type") == "purchase") \
        .map(lambda data: (data.get("product_id"), 1), output_type=Types.TUPLE([Types.STRING(), Types.INT()]))

    top_products = purchase_stream \
        .key_by(lambda x: x[0]) \
        .window(SlidingProcessingTimeWindows.of(Time.seconds(3), Time.seconds(1))) \
        .reduce(lambda a, b: (a[0], a[1] + b[1]))

    top_products.map(lambda x: write_to_redis(x))

    env.execute("Top Products with Sliding Window + Redis")
